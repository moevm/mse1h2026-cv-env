// ExperimentsView.jsx (полностью обновлённый)
...
function ExperimentsView() {
  ...
  const fetchExperiments = async () => {
    setLoading(true);
    try {
      const data = await getExperiments(sortBy, sortOrder);
      setExperiments(data);
    } catch (err) {
  ...
  const handleCompare = async () => {
    ...
    setLoading(true);
    try {
      const response = await compareExperiments(selectedExps);
