...
function TrainingView({ collection, currentVersionId, versions = [] }) {
  ...
  // --- Состояния, подписанные на trainingManager ---
  const [activeTrainings, setActiveTrainings] = useState(() => trainingManager.getActiveTrainings());
  const [metricsData, setMetricsData] = useState(() => {
    const initial = {};
    trainingManager.getActiveTrainings().forEach(t => {
      initial[t.taskId] = trainingManager.getMetrics(t.taskId);
    });
    return initial;
  });
  const [metricsPanelOpen, setMetricsPanelOpen] = useState({});
  const [metricsHistoryOpen, setMetricsHistoryOpen] = useState({});
  const [logs, setLogs] = useState(() => trainingManager.getLogs());

  // --- Вспомогательные функции (форматирование, рендер графиков) ---
  ...
  const getKeyMetric = (training) => {
    ...
    if (latest.mAP50 != null) return { label: 'mAP50', value: latest.mAP50.toFixed(4) };
    if (latest.precision != null) return { label: 'Precision', value: latest.precision.toFixed(4) };
    if (latest.recall != null) return { label: 'Recall', value: latest.recall.toFixed(4) };
    if (training.loss != null) return { label: 'Loss', value: training.loss.toFixed(4) };
    return null;
  };
  ...
  useEffect(() => {
    const unsubscribe = trainingManager.subscribe(() => {
      setActiveTrainings(trainingManager.getActiveTrainings());
      const newMetrics = {};
      trainingManager.getActiveTrainings().forEach(t => {
        newMetrics[t.taskId] = trainingManager.getMetrics(t.taskId);
      });
      setMetricsData(newMetrics);
      setLogs(trainingManager.getLogs());
    });
    return unsubscribe;
  }, []);

  // --- Локальные функции управления ---
  const toggleMetricsPanel = (taskId) => {
    setMetricsPanelOpen(prev => ({ ...prev, [taskId]: !prev[taskId] }));
  };

  const toggleMetricsHistory = (taskId) => {
    setMetricsHistoryOpen(prev => ({ ...prev, [taskId]: !prev[taskId] }));
  };

  const handleStopTraining = async (taskId, modelIdentifier) => {
    try {
      await stopTraining(taskId);
      trainingManager.addLog(`Обучение "${modelIdentifier}" остановлено`, "warning");
    } catch (error) {
      trainingManager.addLog(`Ошибка остановки: ${error.message}`, "error");
    }
  };
  ...
  const validateModelAPI = async (modelName) => {
    if (!modelName.trim()) {
      trainingManager.addLog("Пожалуйста, введите название базовой модели", "warning");
      return false;
    }
    ...
    try {
      ...
      if (data.valid) {
        trainingManager.addLog(`Модель "${modelName}" найдена`, "success");
        return true;
      } else {
        trainingManager.addLog(data.message || `Модель "${modelName}" не найдена`, "error");
        return false;
      }
    } catch (err) {
      trainingManager.addLog(`Ошибка валидации: ${err.message}`, "error");
      return false;
    } finally {
  ...
  const handleSaveConfig = async () => {
    try {
      ...
      trainingManager.addLog("Конфигурация обучения сохранена", "success");
      alert("Конфигурация сохранена");
    } catch (error) {
      trainingManager.addLog(`Ошибка сохранения конфигурации: ${error.message}`, "error");
  ...
  const handleStartTraining = async () => {
    ...
    if (activeFolderNames.length === 0 && !params.use_coco8) {
      trainingManager.addLog("Нет активных папок для обучения. Отметьте папки галочками.", "warning");
    ...
    const trainingData = {
      ...
      modelName: params.modelName && typeof params.modelName === 'string' ? params.modelName : "",
    ...
    try {
      ...
      trainingManager.addTraining(taskInfo);
      trainingManager.addLog(`Запущено обучение "${modelIdentifier}"`, "success");
      trainingManager.addLog(`Task ID: ${taskId}`, "info");
      alert(`Обучение успешно запущено\nTask ID: ${taskId}`);
    } catch (error) {
      trainingManager.addLog(`Ошибка запуска обучения: ${error.message}`, "error");
    ...
    <div className="training-view">
        ...
        <div className="params-left">
          ...
          <div className="param-group checkbox-group" style={{ marginTop: '10px', background: 'rgba(52, 152, 219, 0.1)', padding: '8px', borderRadius: '4px' }}>
            ...
            <input
              ...
              onChange={(e) => setParams(prev => ({ ...prev, use_coco8: e.target.checked }))}
            />
          </div>
      ...
      <div className="console-section">
        <div className="console-header">
          <h3>Логи</h3>
          <button className="clear-console-btn" onClick={clearLogs}>Очистить логи</button>
        </div>
        <div className="console-logs">
          {logs.length === 0 ? (
            <div className="empty-console">Здесь будут отображаться сообщения о процессе обучения</div>
          ) : (
            logs.map((log) => (
